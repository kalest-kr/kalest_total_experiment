# 뉴런 3×3 행렬 · 작은 V1 회로 · 국소 교사 학습 — 실행 결과 보고서 (quick_lrsearch 프로필)

- 생성 시각(UTC): 2026-09-15T03:33:38Z
- 실행 소요 시간: 46.8 초
...
| local_learning | 0.1 | 0.7299 (시드 1개) | 0.4856 (시드 1개) | 0.9703 (시드 1개) | 0.7089 (시드 1개) | 0.6930 (시드 1개) |
...
| local_learning | 42 | 574 | 580 | 39170 | 636 | 0.0146 | 0.5256 | 0.7299 | 0.4856 |
...
| local_learning | 1 | 0.2295 | 시드 1개 | +0.2295 |
...
| local_learning | 0.00000 (시드 1개) | 0.3671 (시드 1개) | 0.2656 (시드 1개) | 7.483 (시드 1개) | 0.01393 (시드 1개) |
...
| local_learning | 46.0 (시드 1개) | 3.0 (시드 1개) | 3.0 (시드 1개) |
...
| local_learning | 1.0000 (시드 1개) | 0.7089 (시드 1개) |
...
|---|---|---|---|---|---|
| 선호 0° | 0.7224 | 0.5254 | 315 | 191 | 378 |
| 선호 90° | 0.7407 | 0.4446 | 259 | 389 | 258 |

**반경 bin 별 시험 성능 (안쪽 0 → 바깥 7)**

| 위치 | balanced acc | F1 | 양성 표본 | TP | FP | FN |
|---|---|---|---|---|---|---|
| 반경 bin 0 | 0.4904 | 0.0000 | 120 | 0 | 96 | 120 |
| 반경 bin 1 | 0.5182 | 0.0529 | 99 | 6 | 122 | 93 |
| 반경 bin 2 | 0.6994 | 0.4421 | 103 | 42 | 45 | 61 |
| 반경 bin 3 | 0.7210 | 0.4153 | 107 | 49 | 80 | 58 |
| 반경 bin 4 | 0.6990 | 0.4225 | 145 | 60 | 79 | 85 |
| 반경 bin 5 | 0.7717 | 0.6073 | 209 | 116 | 57 | 93 |
| 반경 bin 6 | 0.8486 | 0.7398 | 257 | 182 | 53 | 75 |
| 반경 bin 7 | 0.8452 | 0.7062 | 170 | 119 | 48 | 51 |
...
| local_learning | 0.7299 (시드 1개) | 0.6930 (시드 1개) |
...
예시 (`L2/ori0/k0/j0`, trial `diag/1`, tick 2, weight_version 892):
...
- 과다발화(FP)가 가장 많은 위치: **radial_bin_1** (FP=122).
- 측정된 방향 선호가 메타데이터 `preferred_orientation` 과 일치한 L2 채널 **146 / 256**, 측정 불가(반응 동률) 80개. 측정 방향 선호와 미리 지정한 preferred_orientation 메타데이터는 별개다.
...
- frozen: 0.5004 (시드 1개) → local_learning: 0.7299 (시드 1개)
- 짝지은 차이 평균 **0.2295** (시드별 +0.2295)
- F1: frozen 0.0016 (시드 1개) → local_learning 0.4856 (시드 1개)
- 독립 추가 시험(새 대비·새 위치 범위)에서도: frozen 0.5000 (시드 1개) → local_learning 0.6930 (시드 1개)
...
- `local_learning` 은 교정을 제거한 뒤에도 0.7299 (시드 1개) 를 유지한다.
...
- test balanced accuracy 0.5222 (시드 1개) 로 frozen(0.5004 (시드 1개)) 수준에 머문다. local_learning(0.7299 (시드 1개)) 과 뚜렷이 다르다.
...
| local_learning | 0.01393 (시드 1개) | 148 | 0 | 16384 |
...
| local_learning | 42 | 0.000146 | 0.0500 | 0.0506 | 7.483 | 1200 | 892 |
...
|---|---|---|---|---|
| 0 | 0.6131 | 9.14 | 186 | 19.922 |
| 1 | 0.6656 | 9.33 | 185 | 19.924 |
| 2 | 0.6717 | 8.32 | 182 | 19.928 |
| 3 | 0.6856 | 7.41 | 171 | 19.929 |
| 4 | 0.7089 | 6.88 | 168 | 19.933 |
...
**실제 회로의 반사실 개입**: 연결 9654 (`L4/OFF/k5/j10` → `L2/ori0/k4/j9`, excitatory, weight 0.0695) 를 0 으로 바꾸면 관찰 L2 256개 중 **0개**의 발화가 바뀐다.
...
|---|---|---|---|---|---|---|---|
| dev-00000 | horizontal_line | 0° | 1.0000 | 0.0695 | 0 | 0 | -0.0695 |
| dev-00001 | vertical_line | 90° | 1.0000 | 0.0695 | 0 | 0 | -0.0695 |
| dev-00002 | vertical_line | 90° | 0.0000 | 0.0000 | 0 | 0 | 0.0000 |
| dev-00003 | vertical_line | 90° | 0.0000 | 0.0000 | 0 | 0 | 0.0000 |
| dev-00004 | vertical_line | 90° | 1.0000 | 0.0695 | 1 | 0 | -0.0695 |
...
- `results/quick_lrsearch/figures/activity_after_seed42.png`
- `results/quick_lrsearch/figures/activity_before_seed42.png`
- `results/quick_lrsearch/figures/activity_guided_seed42.png`
- `results/quick_lrsearch/figures/condition_summary.png`
- `results/quick_lrsearch/figures/diag_neuron_state_seed42.png`
- `results/quick_lrsearch/figures/input_mapping_seed42.png`
- `results/quick_lrsearch/figures/learning_curve_seed42.png`
- `results/quick_lrsearch/figures/orientation_preference_seed42.png`
...
- 대조 실험 ['frozen', 'correction_only', 'local_learning', 'shuffled_teacher'] × 시드 [42] × 5 에폭: 실행 (총 46.8초)
