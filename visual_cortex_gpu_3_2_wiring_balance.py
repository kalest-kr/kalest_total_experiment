#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""visual_cortex_gpu_3_2_wiring_balance.py -- 입력 쏠림 진단과 배선/강도 균형 대조 실험.
...
실행
----
    python visual_cortex_gpu_3_1_transmission_fix.py                      # 한국어 메뉴
    python visual_cortex_gpu_3_1_transmission_fix.py --mode validate --preset v1_small --device cpu --output "D:\\CortexResults"
    python visual_cortex_gpu_3_1_transmission_fix.py --mode diagnose --preset hierarchy_small --device cuda --output "D:\\CortexResults"
    python visual_cortex_gpu_3_1_transmission_fix.py --mode train    --preset hierarchy_small --train-mode proposed_local_threshold --device cuda --output "D:\\CortexResults"
    python visual_cortex_gpu_3_1_transmission_fix.py --mode resume   --run-dir "D:\\CortexResults\\train_..."
    python visual_cortex_gpu_3_1_transmission_fix.py --mode report   --run-dir "D:\\CortexResults\\train_..." --figures
...
BASE_SOURCE_NAME: str = "visual_cortex_gpu_3_1_transmission_fix.py"
BASE_SOURCE_VERSION: str = "3.1.0"
...
FIX_REGISTER: tuple[FixEntry, ...] = (
    ...
    FixEntry("F41",
             "연결 목록의 양 끝점만 집계하면 **연결이 0 개인 수신 뉴런이 빠져** "
             "쏠림이 과소평가된다.",
             "설정상 송신·수신 후보 전체를 분모로 쓴다. 후보 자체가 없는 경우와 "
             "후보는 있으나 확률 추출로 연결이 없어진 경우를 나눠 보고한다.",
             "check_69_wiring_T8_degenerate_cases"),
    FixEntry("F42",
             "전부 0 인 집단의 CV/Gini 를 0 으로 계산하면 '완전히 균형 잡힘' 으로 "
             "오해된다.",
             "평균이 0 이거나 합이 0 이면 정의 불가(null)로 두고 no_input 상태를 "
             "따로 표시한다. 보고서·그림에서도 n/a 로 찍는다.",
             "check_69_wiring_T8_degenerate_cases"),
    FixEntry("F43",
             "재배선 후 지연을 그대로 두면 거리와 맞지 않는 지연이 남는다.",
             "기존 거리 공식으로 다시 계산하고, 지연·거리 분포의 변화를 조건마다 "
             "보고한다. 개선을 연결 수 분산 하나에 귀속하지 않는다.",
...
ASSUMPTIONS: tuple[Assumption, ...] = (
    ...
    Assumption(
        "wiring_balance_is_engineering_intervention",
        "수신 연결 수·입력 가중치 합의 불균형을 줄이는 배선/강도 개입을 대조군으로 "
        "만든다.",
        "model_choice",
        "공학적 개입이다. 실제 뇌가 동일한 최적화를 수행한다거나 모든 뉴런이 "
        "동일하게 반응해야 한다는 뜻이 아니다. 쏠림이 발견되어도 그것만으로 전달 "
        "실패의 원인이 입증된 것은 아니며, 선택성 때문에 어떤 자극에 반응하지 "
        "않아야 하는 뉴런도 있다."),
    Assumption(
        "wiring_timing_hypothesis_measured_only",
        "H4(도착 시점 분산으로 인한 순간 탈분극 부족)는 이번에 **측정만** 한다.",
        "untested_hypothesis",
        "발화 시각·전도 속도·시냅스 시상수를 임의로 동기화하지 않는다. 재배선이 "
        "지연 분포를 바꾸는 것은 기존 거리 공식의 결과이며 의도적 동기화가 아니다."),
    Assumption(
        "rewiring_changes_delay_distribution",
        "C2/C4 의 재배선 후 지연을 기존 거리 공식으로 다시 계산한다.",
        "model_choice",
        "따라서 재배선은 지연·거리 분포도 바꾼다. 개선을 연결 수 분산 하나에 전부 "
        "귀속하지 않는다. sham(C1) 과의 차이를 함께 보고한다."),
    Assumption(
...
IMPLEMENTATION_STATUS: tuple[ImplementationItem, ...] = (
    ...
    ImplementationItem("readout_refit_at_high_inhibition", "not_implemented",
                       ...
                       "기록한다. 재적합은 별도 실험이다."),
    ImplementationItem("wiring_structure_diagnosis", "implemented",
                       "후보 집합 전체를 분모로 in/out degree·가중치 합·거리·지연 "
                       "분포와 CV/Gini 를 낸다. 전부 0 인 집단은 정의 불가로 "
                       "표시한다."),
    ImplementationItem("wiring_activity_diagnosis", "implemented",
                       "엔진과 같은 지연 링·수용체·구획 인덱스로 경로별 도착량을 "
                       "수신 뉴런별로 모은다. 경로 성분 합을 엔진 전체 합과 "
                       "대조한다."),
    ImplementationItem("wiring_conditions_C0_to_C4", "implemented",
                       "C1 이중 교환, C2 수신 끝점 재배치, C3 제한 정규화, C4 결합을 "
                       "구현했다. 달성 못한 제약은 unmet_constraints 로 보고한다."),
    ImplementationItem("wiring_fixed_source_reaggregation", "implemented",
                       "baseline 송신열을 고정해 각 조건의 배선으로 도착량만 다시 "
                       "센다. 새 발화를 만들지 않으며 이것만으로 막전위·발화 개선을 "
                       "주장하지 않는다."),
    ImplementationItem("wiring_H4_timing_intervention", "not_implemented",
                       "H4 는 측정만 한다. 발화 시각·전도 속도·시냅스 시상수를 "
                       "동기화하는 개입은 이번 범위가 아니다."),
    ImplementationItem("wiring_isolated_source_rescue", "not_implemented",
                       "baseline 에서 완전히 고립된 송신 뉴런은 out_degree 보존 "
                       "조건에서 구제할 수 없다. 별도 실험 과제로 남긴다."),
    ImplementationItem("wiring_input_timeseries_export", "not_implemented",
                       "선택형 시계열 내보내기는 상한만 설정에 두었고 파일 저장은 "
                       "구현하지 않았다. 요약 지표와 뉴런별 CSV 는 항상 저장한다."),
    ImplementationItem("wiring_resume", "not_implemented",
                       "배선 실험은 조건 단위로 원자적으로 저장하지만 중간 재개를 "
                       "지원하지 않는다. 완료/미완료 조건을 구분해 보고한다."),
    ImplementationItem("wiring_validation_T1_to_T9", "unverified",
                       "명세 9절 T1~T9 (check_62~70) 을 코드로 구현했다. 작성 "
                       "환경에 torch·GPU 가 없어 실행 검증하지 않았다: 사용자가 "
                       "메뉴 13-D 로 실행해야 PASS/FAIL 이 정해진다. GPU 경로는 "
                       "미검증이다."),
...
class CorticalBuilder:
    ...
    def _wire(self, arrays: NeuronArrays, meta: dict[str, Any]
        ...
        if profile:
            validate_transmission_profile(cfg)
            if tensor_hash(syn.w0_nS) != profile["baseline_w0_sha256"]:
                raise ValueError("전달 보정 기준 w0가 다르다. 같은 시드/버전에서 재보정하라.")
            calib["legacy_per_rule_factor"] = dict(calib["per_rule_factor"])
            calib["transmission_rule_gains"] = dict(profile["rule_gains"])
            for r_idx, rule in enumerate(cfg["wiring"]["rules"]):
                gain = float(profile["rule_gains"].get(rule["name"], 1.0))
                if gain != 1.0:
                    mask = syn.rule_index == r_idx
                    if not bool((arrays.dale[syn.src[mask]] > 0).all()):
                        raise ValueError("억제성 시냅스는 전달 보정 대상이 아니다.")
                    syn.w0_nS[mask] = syn.w0_nS[mask] * gain
                if rule["name"] in calib["per_rule_factor"]:
                    calib["per_rule_factor"][rule["name"]] *= gain
            if tensor_hash(syn.w0_nS) != profile["final_w0_sha256"]:
                raise ValueError("프로필 적용 후 w0 해시가 저장된 채택값과 다르다.")
            for row in calib["rows"]:
                gain = float(profile["rule_gains"].get(row["rule"], 1.0))
                row["transmission_gain"] = gain
                row["factor"] *= gain
                row["w0_mean_calibrated_nS"] *= gain
            calib["note_ko"] += " train-only 동적 전달 초기화 배율을 추가 적용했다."
            self.notes.append("실제 엔진으로 보정한 감각 연결 배율을 적용했다. 과제 학습 중 w0는 고정이다.")
        ...
        if area in cortical_order:
            for stage in ("L4_excitatory", "L23_excitatory"):
                ...
                if row.get("n_neurons", 0) == 0:
                    ...
                    reason = ("basal_input_without_spikes" if row.get("basal_exc_arrival_sum_nS", row.get("arrival_basal_excitatory_nS_sum", 0)) > 0
...
def gini_coefficient(values: np.ndarray) -> float | None:
    """Gini 계수. 전부 0 이면 **정의 불가**이므로 None 을 돌려준다.

    전부 0 인 집단을 '완전히 균형 잡힘' 으로 표시하지 않는다. 호출한 쪽이
    ``no_input`` 상태를 따로 적어야 한다.
    """
    v = np.asarray(values, dtype=np.float64)
    if v.size == 0:
        return None
    if np.any(v < 0):
        raise ValueError("Gini 는 비음수 값에만 쓴다.")
    total = float(v.sum())
    if total <= 0.0:
        return None                      # 전부 0 -> 정의 불가
    x = np.sort(v)
    n = x.size
...
def coefficient_of_variation(values: np.ndarray) -> float | None:
    ...
    if mean == 0.0:
        return None
    return float(v.std(ddof=0) / mean)


def distribution_summary(values: np.ndarray, *, name: str) -> dict[str, Any]:
    """min/median/mean/max 와 분포, CV/Gini. 전부 0 이면 그 사실을 표시한다."""
    v = np.asarray(values, dtype=np.float64)
    if v.size == 0:
        return {"name": name, "n": 0, "status": "empty",
                "note_ko": "이 집단에 뉴런이 없다"}
    all_zero = bool(np.all(v == 0))
    hist, edges = np.histogram(v, bins=min(20, max(1, int(v.size))))
    return {
        ...
        "all_zero": all_zero,
        "cv": coefficient_of_variation(v),
        "gini": gini_coefficient(v),
        "histogram_counts": [int(x) for x in hist],
        "histogram_edges": [float(x) for x in edges],
        "status": "no_input" if all_zero else "ok",
        "note_ko": ("전부 0 인 집단이다. CV/Gini 는 정의 불가(null)이며 이것을 "
                    "'좋은 균형' 으로 읽으면 안 된다." if all_zero else None),
    }
...
class GroupArrivalTracker:
    ...
    def __init__(self, model: CorticalModel, groups: Sequence["ConnectionGroup"],
        ...
        dt = float(model.cfg["engine"]["dt_ms"])
        self.decay = t.tensor(
            [math.exp(-dt / float(RECEPTOR_PARAMS[r]["tau_ms"])) for r in RECEPTORS],
            dtype=dtp, device=dev)
    ...
    def observe(self, info: dict[str, Any]) -> None:
        ...
        for name, lst in self.per_group.items():
            ...
            self.running_g[name] = self.running_g[name] * float(
                self.decay[RECEPTOR_INDEX[self._receptor_of(name)]]) + step_g
        ...
        self.all_edges_sum += dg.sum(dim=(-1, -2))
        st = self.model.state
        ...
        self.peak_soma_mV = t.maximum(self.peak_soma_mV,
                                      st.V[0, 0, :, COMP_INDEX["soma"]])
    ...
    def consistency_check(self) -> dict[str, Any]:
        """경로별 성분의 합이 전체 엔진 값과 맞는지 (명세 5절 / T9).

        선택한 연결군만 추적하므로 **부분 합 <= 전체 합** 이어야 한다. 모든
        연결군을 추적하면 같아야 한다.
        """
        t = require_torch()
        part = t.zeros_like(self.all_edges_sum)
        for name in self.arrival_sum:
            part += self.arrival_sum[name]
        total = float(self.all_edges_sum.sum())
        partial = float(part.sum())
        over = float((part - self.all_edges_sum).max())
        n_groups_tracked = len(self.arrival_sum)
        n_rules = len(self.model.cfg["wiring"]["rules"])
        return {
            "tracked_groups": n_groups_tracked, "total_rules": n_rules,
            "tracks_all_rules": n_groups_tracked == n_rules,
            "engine_total_arrival_nS": total,
            "tracked_partial_sum_nS": partial,
            "max_per_neuron_excess_nS": over,
            "partial_not_exceeding_total": bool(over <= 1e-6 * max(1.0, abs(total))),
            "equal_when_all_tracked": (bool(abs(partial - total)
                                            <= 1e-6 * max(1.0, abs(total)))
                                       if n_groups_tracked == n_rules else None),
            "note_ko": ("뉴런별 합과 전체 엔진 합을 대조한다. 층별 합계만으로 "
                        "통과시키지 않는다. 일부 연결군만 추적하면 부분 합이 전체 "
                        "합보다 작은 것이 정상이다."),
...
def load_transmission_config(path: Path) -> dict[str, Any]:
    """저장한 calibrated_config.json을 읽는다. 배열을 pickle로 복원하지 않는다."""
    cfg = read_json(path)
...
class ExperimentRunner:
        ...
        for g in groups:
            ...
            by_group[g.name] = {
                ...
                "peak_u_minus_theta": distribution_summary(
                    margin, name="peak_u_minus_theta"),
    ...
    def run_wiring_compare(self, *, group_name: str | None = None,
        ...
        seed_list = [int(x) for x in (seeds or [int(self.cfg["seed"])])]
        gain = float(wb["inhibition_gain"] if inhibition_gain is None
                     else inhibition_gain)
        if self.run_dir is None:
            self.setup(tag)
    ...
    @staticmethod
    def _wiring_constraint_achievement(per_cond: dict[str, Any]) -> dict[str, Any]:
        """조건별 제약 **달성/미달**을 manifest 한 곳에 모은다 (명세 10절).

        미달을 조용히 숨기지 않는다. 자세한 근거는 ``wiring_compare.json`` 에 있다.
        """
        rows: dict[str, Any] = {}
        any_unmet = False
        for cond, row in per_cond.items():
            iv = row.get("intervention") or {}
            unmet = list(iv.get("unmet_constraints") or [])
            any_unmet = any_unmet or bool(unmet)
            entry: dict[str, Any] = {
                "applied": bool(iv.get("applied", cond == "C0_baseline")),
                "n_unmet": len(unmet),
                "unmet_codes": [u.get("code") for u in unmet],
            }
            for k in ("requested_min_in_degree", "achieved_min_in_degree",
                      "termination", "hit_iteration_cap",
                      "total_weight_preserved", "total_weight_residual_nS",
                      "converged_to_fixed_point", "bounds_multiplier",
                      "n_dst_with_zero_input"):
                if k in iv:
                    entry[k] = iv[k]
            rows[cond] = entry
        return {
            "per_condition": rows,
            "any_constraint_unmet": any_unmet,
            "note_ko": ("미달 제약은 몰래 풀지 않고 그대로 남긴다. 미달이 있으면 "
                        "그 조건의 결과를 '균형화 달성'으로 읽지 마라. 사유별 뜻은 "
                        "WIRING_UNMET_REASONS 에 있다."),
            "reasons_ko": dict(WIRING_UNMET_REASONS),
        }

    @staticmethod
    def _wiring_preservation_checks(per_cond: dict[str, Any]) -> dict[str, Any]:
        """조건별 **보존 대상과 변경 대상**을 구분해 확인한다 (T6)."""
        base = per_cond.get("C0_baseline")
        if base is None:
            return {"status": "no_baseline"}
        bt = base["group_totals"]
        out: dict[str, Any] = {}
        for cond, row in per_cond.items():
            if cond == "C0_baseline":
                continue
            ct = row["group_totals"]
            exp: dict[str, Any] = {
                "n_edges_preserved": ct["n_edges"] == bt["n_edges"],
                "other_groups_untouched": row["intervention"][
                    "other_groups_untouched"],
            }
            if cond == "C1_sham_rewire":
                exp["in_degree_preserved_expected"] = True
                exp["in_degree_preserved"] = (ct["in_degree_hash"]
                                              == bt["in_degree_hash"])
                exp["out_degree_preserved"] = (ct["out_degree_hash"]
                                               == bt["out_degree_hash"])
                exp["total_weight_preserved"] = bool(
                    abs(ct["total_weight_nS"] - bt["total_weight_nS"]) <= 1e-9)
            elif cond in ("C2_degree_balanced", "C4_degree_and_strength"):
                exp["in_degree_changed_expected"] = True
                exp["in_degree_changed"] = (ct["in_degree_hash"]
                                            != bt["in_degree_hash"])
                exp["out_degree_preserved"] = (ct["out_degree_hash"]
                                               == bt["out_degree_hash"])
                exp["source_output_weight_preserved"] = (
                    ct["source_output_weight_hash"]
                    == bt["source_output_weight_hash"]
                    if cond == "C2_degree_balanced" else None)
                exp["total_weight_preserved"] = bool(
                    abs(ct["total_weight_nS"] - bt["total_weight_nS"])
                    <= 1e-6 * max(1.0, bt["total_weight_nS"]))
            elif cond == "C3_strength_balanced":
                exp["wiring_preserved_expected"] = True
                exp["in_degree_preserved"] = (ct["in_degree_hash"]
                                              == bt["in_degree_hash"])
                exp["out_degree_preserved"] = (ct["out_degree_hash"]
                                               == bt["out_degree_hash"])
                exp["total_weight_preserved"] = bool(
                    abs(ct["total_weight_nS"] - bt["total_weight_nS"])
                    <= 1e-6 * max(1.0, bt["total_weight_nS"]))
                exp["delay_unchanged"] = (row["intervention"]["delay_change"]
                                          ["n_delay_changed"] == 0)
            out[cond] = exp
        out["note_ko"] = ("각 조건에서 **의도된 차이**를 오류로 오인하지 않도록 "
                          "보존 대상과 변경 대상을 나눠서 본다.")
        return out

    @staticmethod
    def _wiring_paired(results: dict[str, Any],
                       conditions: Sequence[str]) -> dict[str, Any]:
        """같은 시드 baseline 대비 차이 + 조건별 평균·표준편차."""
        rows: list[dict[str, Any]] = []
        for seed, one in results.items():
            if one.get("status") != "completed":
                rows.append({"seed": seed, "status": one.get("status"),
                             "reason": one.get("error"),
                             "note_ko": "실패한 시드를 표에서 빼지 않는다"})
                continue
            conds = one["conditions"]
            base = conds.get("C0_baseline")
            if base is None:
                continue
            b = base["activity_summary"]
            for cond in conditions:
                if cond not in conds:
                    continue
                c = conds[cond]["activity_summary"]
                gname = next(iter(b["by_group"]), None)
                bg = b["by_group"].get(gname, {}) if gname else {}
                cg = c["by_group"].get(gname, {}) if gname else {}
                row = {
                    "seed": seed, "condition": cond, "group": gname,
                    "delivered_fraction": c["delivery"]["delivered_fraction"],
                    "delivered_fraction_baseline":
                        b["delivery"]["delivered_fraction"],
                    "delta_delivered_fraction":
                        c["delivery"]["delivered_fraction"]
                        - b["delivery"]["delivered_fraction"],
                    "fraction_no_arrival": cg.get("fraction_no_arrival"),
                    "delta_fraction_no_arrival":
                        (cg.get("fraction_no_arrival", 0.0)
                         - bg.get("fraction_no_arrival", 0.0)),
                    "fraction_arrived_but_no_spike":
                        cg.get("fraction_arrived_but_no_spike"),
                    "delta_fraction_arrived_but_no_spike":
                        (cg.get("fraction_arrived_but_no_spike", 0.0)
                         - bg.get("fraction_arrived_but_no_spike", 0.0)),
                    "fraction_spiked": cg.get("fraction_spiked"),
                    "delta_fraction_spiked": (cg.get("fraction_spiked", 0.0)
                                              - bg.get("fraction_spiked", 0.0)),
                    "in_degree_cv": conds[cond]["group_totals"].get("in_degree_cv"),
                    "in_degree_gini":
                        conds[cond]["group_totals"].get("in_degree_gini"),
                    "status": "ok",
                }
                rows.append(row)
        agg: dict[str, Any] = {}
        for cond in conditions:
            sel = [r for r in rows if r.get("condition") == cond
                   and r.get("status") == "ok"]
            if not sel:
                agg[cond] = {"n_seeds": 0, "status": "no_data"}
                continue
            def stat(key: str) -> dict[str, Any]:
                ...
                return {"mean": float(np.mean(v)) if v else None,
                        "std": float(np.std(v, ddof=1)) if len(v) >= 2 else None,
                        "n": len(v)}
            agg[cond] = {
                "n_seeds": len(sel),
                "delta_delivered_fraction": stat("delta_delivered_fraction"),
                "delta_fraction_no_arrival": stat("delta_fraction_no_arrival"),
                "delta_fraction_spiked": stat("delta_fraction_spiked"),
                "delta_fraction_arrived_but_no_spike":
                    stat("delta_fraction_arrived_but_no_spike"),
            }
        return {"rows": rows, "by_condition": agg,
                "counting_note_ko": ("개별 뉴런·시냅스·시간 스텝을 독립 실험 반복 "
                                     "수로 세지 않는다. n 은 시드 수다."),
                "note_ko": ("같은 시드 baseline 대비 차이다. 시드가 적으면 표준편차를 "
                            "신뢰하지 마라.")}

    def _write_paired_csv(self, root: Path, paired: dict[str, Any]) -> None:
        cols = ("seed", "condition", "group", "delivered_fraction",
                "delivered_fraction_baseline", "delta_delivered_fraction",
                "fraction_no_arrival", "delta_fraction_no_arrival",
                "fraction_arrived_but_no_spike",
                "delta_fraction_arrived_but_no_spike", "fraction_spiked",
                "delta_fraction_spiked", "in_degree_cv", "in_degree_gini",
                "status", "reason")
        path = root / "paired_comparisons.csv"
        with path.open("w", encoding="utf-8", newline="") as fh:
            w = csv.DictWriter(fh, fieldnames=list(cols), extrasaction="ignore")
            w.writeheader()
            for r in paired.get("rows", []):
                w.writerow(r)

    # ------------------------------------------------------------------
    def run_wiring_bundle(self, *, groups: Sequence[str] | None = None,
                          conditions: Sequence[str] | None = None,
                          seeds: Sequence[int] | None = None,
                          inhibition_gain: float | None = None
                          ) -> dict[str, Any]:
        ...
        같은 baseline 에서 연결군 하나씩을 바꾸며, 첫 번째 변경을 다음 조건에
        **누적하지 않는다**. 전체 연결군을 동시에 고쳐 어느 경로가 영향을 줬는지
        알 수 없는 결과를 내지 않는다.
        """
        require_torch()
        self.setup("wiring_bundle")
        root = self.run_dir
        assert root is not None
        wb = self.cfg["wiring_balance"]
        gain = float(wb["inhibition_gain"] if inhibition_gain is None
                     else inhibition_gain)
        # 대상 연결군 이름 확인 (모델을 한 번 만들어 실제 존재 여부를 본다)
        probe = self.build(with_recorder=False)
        self.build_data()
        gl, missing = resolve_connection_groups(probe, groups)
        names = [g.name for g in gl]
        del probe
        self.model = None
        out: dict[str, Any] = {
            "task": "wiring-bundle", "root": str(root),
            "groups_resolved": names, "groups_missing": missing,
            "conditions": list(conditions or wb["conditions"]),
            "seeds": [int(x) for x in (seeds or [int(self.cfg["seed"])])],
            "inhibition_gain": gain,
            "one_group_at_a_time": True,
            "cumulative": False,
            "design_note_ko": ("같은 baseline 에서 연결군 하나씩만 바꾼다. 앞 조건의 "
                               "변경을 다음 조건에 누적하지 않는다."),
            "per_group": {}, "failures": [],
        }
        for nm in names:
            self.recorder = None
            sub = ExperimentRunner(self.cfg, run_dir=root / safe_name(f"group_{nm}"),
                                   device_choice=self.device_choice, env=self.env)
            sub.setup(safe_name(nm))
            try:
                res = sub.run_wiring_compare(
                    group_name=nm, conditions=conditions, seeds=seeds,
                    inhibition_gain=gain, tag=safe_name(nm))
            except Exception as exc:                      # noqa: BLE001
                res = {"group": nm, "status": "failed",
                       "error": f"{type(exc).__name__}: {exc}",
                       "traceback": traceback.format_exc(),
                       "note_ko": "실패한 연결군을 표에서 빼지 않는다"}
                out["failures"].append({"group": nm, "error": res["error"]})
            out["per_group"][nm] = {k: v for k, v in res.items()
                                    if k not in ("per_seed",)}
            out["per_group"][nm]["run_dir"] = str(sub.run_dir)
        write_json(root / "bundle_summary.json", out)
        return out

    # ------------------------------------------------------------------
    def run_wiring_integrity(self) -> dict[str, Any]:
        """D — 이 기능의 무결성 검사 (T1~T9). 사용자가 실행한다."""
        require_torch()
        self.setup("wiring_integrity")
        assert self.run_dir is not None
        suite = ValidationSuite(self.cfg, self.device, self.dtype,
                                progress=lambda m: None)
        names = [n for n in dir(suite) if n.startswith("check_")
                 and "wiring" in n]
        res = suite.run_all(only=names)
        res["scope"] = "wiring_balance_integrity_T1_T9"
        res["checks_selected"] = sorted(names)
        write_json(self.run_dir / "wiring_integrity.json", res)
        return res
...
class ValidationSuite:
    ...
    def check_49_inhibition_alpha_one_regression(self) -> CheckResult:
        """배율 1의 실제 항등 경로 참조. 자발 억제 세포가 침묵해도 검사가 성립한다."""
        n_steps = int(round(self.tiny_cfg["engine"]["sample_ms"]
                            / self.tiny_cfg["engine"]["dt_ms"]))
        stim = self._tiny_stim("shape")
        def measure(alpha: float, bypass: bool = False) -> dict[str, Any]:
            cfg = copy.deepcopy(self.tiny_cfg)
            cfg["deterministic"] = True
            m = self._fresh_model(cfg)
            m.set_inhibition_gain(alpha, reason="check_49")
            if bypass:
                m.engine.inhibition.apply_to_emission = lambda q: q
            drive, _ = self._prepared(m, stim, n_steps)
            out = m.run(drive, m.gains.replicas(None), n_steps)
        ...
        one = measure(1.0)
        identity = measure(1.0, bypass=True)
        low = measure(0.01)
        identical = one == identity
        return CheckResult(
            "check_49_inhibition_alpha_one_regression",
            "alpha=1의 항등 게이트 참조 동등성; 자발 배율 효과는 별도 관측",
            STATUS_PASSED if identical else STATUS_FAILED,
            "결정론 경로에서 alpha=1 게이트와 게이트 항등 우회의 전압/전도도/발화가 같다",
            {"alpha1_matches_identity_reference": identical,
             "alpha1_vs_alpha001_differs": one != low,
             "alpha1_spikes": one["n_spikes"], "alpha001_spikes": low["n_spikes"]},
            "tiny preset, 실제 엔진; alpha 효과의 인과 검사는 check_50/51/53",
            {"note_ko": "입력에서 억제 뉴런이 발화하지 않으면 두 배율의 출력이 같을 수 있다. "
                        "이 관측을 게이트 구현 실패로 판정하지 않는다. 이전 버전 전체 회귀 증명도 아니다."})
    ...
    # ==================================================================
    # 배선/강도 균형 실험 무결성 검사 (명세 9절 T1~T9)
    # ==================================================================
    ...
    def check_64_wiring_T3_zero_input_and_reset(self) -> CheckResult:
        """T3 — zero-input 에서 비정상 입력이 생기지 않고, 초기화 후 이전 자극이 안 섞인다."""
        t = require_torch()
        m = self._wiring_model()
        n_steps = 6
        # 1) 완전 무입력
        ...
        zero_drive = ExternalDrive(
            ...
            values=t.zeros((n_steps, 1, int(m.retina_neuron_ids.numel())),
                           dtype=self.dtype, device=self.device),
            meta={"drive_unit": "nS"})
        P = m.gains.replicas(None)
        out0 = m.run(zero_drive, P, n_steps)
        zero_spikes = int(out0["spike_count"].sum())
        zero_g = float(m.state.g.sum())
        # 2) 실제 자극 -> 3) 초기화 -> 4) 다시 무입력
        stim = self._tiny_stim("shape")
        drive, _ = self._prepared(m, stim, n_steps)
        m.run(drive, P, n_steps)
        after_stim_g = float(m.state.g.sum())
        m.reset_dynamics()
        cleared_g = float(m.state.g.sum())
        cleared_ring = float(m.ring.buf.abs().sum())
        out2 = m.run(zero_drive, P, n_steps)
        zero_again = int(out2["spike_count"].sum())
        passed = (zero_spikes == 0 and zero_g == 0.0 and cleared_g == 0.0
                  and cleared_ring == 0.0 and zero_again == 0)
        return CheckResult(
            "check_64_wiring_T3_zero_input_and_reset",
            "무입력에서 입력이 생기지 않고 초기화가 이전 자극을 지운다",
            STATUS_PASSED if passed else STATUS_FAILED,
            "무입력 스파이크 0, 전도도 0, 초기화 후 전도도·링 버퍼 0, 자극 이후 "
            "다시 무입력을 넣어도 스파이크 0",
            {"zero_input_spikes": zero_spikes,
             "zero_input_conductance": zero_g,
             "conductance_after_stimulus": after_stim_g,
             "conductance_after_reset": cleared_g,
             "ring_after_reset": cleared_ring,
             "zero_input_spikes_after_stimulus_and_reset": zero_again},
            "tiny preset 실제 엔진")
    ...
    def check_66_wiring_T5_baseline_path_equivalence(self) -> CheckResult:
        ...
        # 계측 경로 vs 엔진 전체 합
        stim = self._tiny_stim("shape")
        drive, _ = self._prepared(m, stim, 6)
        P = m.gains.replicas(None)
        all_groups = [ConnectionGroup(m, i)
                      for i in range(len(m.cfg["wiring"]["rules"]))]
        all_groups = [g for g in all_groups if g.n_edges > 0]
        tracker = GroupArrivalTracker(m, all_groups)
        m.run(drive, P, 6, step_hook=tracker.observe)
        cons = tracker.consistency_check()
        passed = (all(same.values()) and built["n_changed_edges"] == 0
                  and cons["partial_not_exceeding_total"]
                  and (cons["equal_when_all_tracked"] is not False))
        return CheckResult(
            "check_66_wiring_T5_baseline_path_equivalence",
            ...
            {"synapse_hashes_unchanged": same,
             ...
             "consistency": cons, "n_groups_tracked": len(all_groups)},
    ...
    def check_70_wiring_T9_per_neuron_vs_engine_total(self) -> CheckResult:
        """T9 — **뉴런별 합**과 전체 엔진 합을 대조한다 (층별 합계만으로 통과시키지 않는다)."""
        t = require_torch()
        m = self._wiring_model()
        groups = [ConnectionGroup(m, i)
                  for i in range(len(m.cfg["wiring"]["rules"]))]
        ...
        m.run(drive, P, 6, step_hook=tracker.observe)
        part = t.zeros_like(tracker.all_edges_sum)
        for name in tracker.arrival_sum:
            part += tracker.arrival_sum[name]
        per_neuron_diff = (part - tracker.all_edges_sum).abs()
        worst = float(per_neuron_diff.max())
        scale = max(float(tracker.all_edges_sum.abs().max()), 1e-12)
        rel = worst / scale
        layer_only = float(abs(float(part.sum())
                               - float(tracker.all_edges_sum.sum())) / scale)
        tol = 1e-5 if str(self.device).startswith("cuda") else 1e-6
        passed = rel <= tol
        return CheckResult(
            "check_70_wiring_T9_per_neuron_vs_engine_total",
            "뉴런별 계측 합이 엔진 전체 합과 일치 (합계만으로 통과시키지 않는다)",
            STATUS_PASSED if passed else STATUS_FAILED,
            f"뉴런별 최대 상대 오차 <= {tol}. 전체 합만 맞고 개별 뉴런이 틀린 경우를 "
            f"잡기 위해 per-neuron 최대값을 본다",
            {"max_per_neuron_relative_difference": rel,
             "max_per_neuron_absolute_difference": worst,
             "sum_only_relative_difference": layer_only,
             "tolerance": tol, "n_groups_tracked": len(groups),
             "tracks_all_rules": tracker.consistency_check()["tracks_all_rules"]},
            "tiny preset, 전체 연결군 추적 후 뉴런별 대조")
...
class KoreanMenu:
    ...
    def dispatch(self, choice: str) -> None:
        cfg = (copy.deepcopy(self.calibrated_cfg) if self.calibrated_cfg is not None
               else build_config(self.preset))
        out = self.output
        ...
        if choice == "1":
            ...
            print(f"  '{choice}' 은(는) 없는 번호다. 0~13 중에서 고르라.")

    # ------------------------------------------------------------------
    def wiring_menu(self, cfg: dict[str, Any], out: Path) -> None:
        """13번 하위 메뉴 (A~E). 고른 작업만 수행한다."""
        wb = cfg["wiring_balance"]
        while True:
            ...
            sub = _ask("A/B/C/D/E 중 선택 (0=뒤로)", "0").strip().upper()
            if sub in ("0", ""):
                return
            if sub == "A":
                ...
                print("  [예상 작업량] tiny preset 으로 T1~T9 검사만 실행한다 "
                      "(학습·본 실험 없음).")
                if not self._confirm():
                    continue
                runner = ExperimentRunner(cfg, output_root=out,
                ...
                print(f"  '{sub}' 은(는) 없는 선택이다. A/B/C/D/E 또는 0 을 고르라.")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "쓰지 않는다.\n"
            "\n배선/강도 균형 대조 실험 예시:\n"
            f"  python {Path(__file__).name} --mode wiring-diagnose "
            "--preset hierarchy_small --device cuda "
            "--output \"D:\\CortexResults\"\n"
            f"  python {Path(__file__).name} --mode wiring-compare "
            "--preset hierarchy_small --device cuda "
            "--wiring-group \"V1->V2_FF\" --wiring-min-in-degree 2 "
            "--output \"D:\\CortexResults\"\n"
            f"  python {Path(__file__).name} --mode wiring-bundle "
            "--preset hierarchy_small --device cuda "
            "--wiring-seeds 20260101,20260102,20260103,20260104,20260105 "
            "--output \"D:\\CortexResults\"\n"
            f"  python {Path(__file__).name} --mode wiring-integrity "
            "--preset tiny --device cpu --output \"D:\\CortexResults\"\n"
            f"  python {Path(__file__).name} --mode report "
            "--run-dir \"D:\\CortexResults\\wiring_compare_...\" --figures\n"
